...
async def new_quiz(message):
    ...
    await DB.result_right(user_id, new_right)
...
async def right_answer(callback: types.CallbackQuery):
    ...
    #Запись кол-ва правильных ответов
    right = await DB.get_result_right(callback.from_user.id)
    if right is None:
        right = 0
    ...
    await DB.result_right(callback.from_user.id, right)
    ...
    if current_question_index < len(quiz_data):
        ...
        await callback.message.answer(f"Это был последний вопрос. Квиз завершен!\nВаш результат: {right} правильных ответов")
